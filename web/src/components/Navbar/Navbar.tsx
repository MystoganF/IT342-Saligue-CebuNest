...
const Navbar: React.FC<NavbarProps> = ({ user }) => {
  ...
  const markRead = async (notif: AppNotification) => {
    ...
    setNotifOpen(false);
    if (isAdminBroadcast(notif.type)) setSelectedAdminNotif(notif);
    else {
      ...
      <nav className={styles.navbar}>
        <div className={styles.inner}>
          ...
          <div className={styles.navLinks}>
            <Link to="/home" className={`${styles.navLink} ${isActive("/home")}`}>Browse</Link>
            <Link to="/my-rentals" className={`${styles.navLink} ${isActive("/my-rentals")}`}>My Rentals</Link>
          </div>
                ...
                <div className={styles.notifDropdown} role="dialog" aria-label="Notifications">
                  ...
                  {notifications.length > 0 && (
                    <div className={styles.notifDropdownFooter}>
                      <button className={styles.viewAllBtn} onClick={() => { setNotifOpen(false); navigate("/my-rentals"); }} type="button">View all rentals →</button>
                    </div>
                  )}
                </div>
              )}
              ...
              {menuOpen && (
                <div className={styles.dropdown} role="menu">
                  ...
                  <div className={styles.dropdownItems}>
                    <button className={styles.dropdownItem} role="menuitem" onClick={() => { setMenuOpen(false); navigate("/tenant/profile"); }}><User size={16} className={styles.dropdownItemIcon} /> Profile</button>
      ...
      {showLogoutModal && (
        <div className={styles.modalOverlay} onClick={() => setShowLogoutModal(false)}>
          <div className={styles.modal} onClick={(e) => e.stopPropagation()}>
            ...
            <p className={styles.modalBody}>You'll be logged out of your account and returned to the login page.</p>
