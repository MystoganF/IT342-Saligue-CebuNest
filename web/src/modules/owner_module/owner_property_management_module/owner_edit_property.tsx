...
interface RentalRequest {
  ...
  status: string;
  createdAt: string;
  tenantFacebookUrl?: string | null;
  tenantInstagramUrl?: string | null;
...
function paymentStatusColor(status: string): {
  ...
  switch (status) {
    case "PAID":
      return { color: "#1a7a4a", bg: "#e8f7ef", border: "rgba(26,122,74,0.2)" };
    case "OVERDUE":
      return { color: "#c0392b", bg: "#fdf0ee", border: "rgba(192,57,43,0.2)" };
    case "PENDING":
      return { color: "#b78e42", bg: "#fffbea", border: "rgba(183,142,66,0.2)" };
    default:
      return { color: "#6e7071", bg: "#f0f4f5", border: "#e5eced" };
  }
}
...
// ─── Component Helper for Social Links ─────────────────────────────────────
...
const EditProperty: React.FC = () => {
  ...
  const fetchAllPropertyData = useCallback(async () => {
    ...
    try {
      ...
      ownerApi
        ...
        .then((data) => {
          if (data.success) {
            ...
            setRequests(reqs);
            if (reqs.length > 0)
              setOpenRequestYears(new Set([getYear(reqs[0].createdAt)]));
      ...
      const activeTenantData = await ownerApi
        .getActiveTenant(id)
        .catch(() => ({ success: false }));

      if (
        activeTenantData.success &&
        activeTenantData.data?.activeTenant?.tenantId
      ) {
        ...
        const tenant = {
          ...
          status: t.status,
          tenantFacebookUrl: t.tenantFacebookUrl ?? null,
          tenantInstagramUrl: t.tenantInstagramUrl ?? null,
          tenantTwitterUrl: t.tenantTwitterUrl ?? null,
        };
        ...
        ownerApi
          .getPaymentsForRequest(tenant.id)
          .then((data) => {
            if (data.success) {
        ...
        ownerApi
          .getLeaseExtensions(tenant.id)
          .then((data) => {
            if (data.success)
              setLeaseExtensions(data.data.extensionRequests ?? []);
          })
          .catch(() => {})
  ...
  // ── Past Request Modal Payments Effect ─────────────────────────────────
  useEffect(() => {
    if (!pastRequestModal) {
      setModalPayments([]);
      setModalOpenPaymentYears(new Set());
      return;
    }

    setModalPaymentsLoading(true);
    ownerApi
      .getPaymentsForRequest(pastRequestModal.id)
      .then((data) => {
        if (data.success) {
          const pmts = data.data.payments ?? [];
          ...
          if (pmts.length > 0) {
            const autoYear = getYear(pmts[pmts.length - 1].dueDate);
            setModalOpenPaymentYears(new Set([autoYear]));
          }
        }
      })
      .catch(() => {})
      .finally(() => setModalPaymentsLoading(false));
  }, [pastRequestModal]);
  ...
  const handleRequestAction = async () => {
    ...
    setActionError(null);
    try {
      const data = await ownerApi.updateRentalRequestStatus(
        actionTarget.id,
        actionType,
      );
      if (!data.success) {
      ...
      if (actionType === "APPROVED") {
        ...
        setRequests((prev) =>
          prev.map((r) =>
            r.id === actionTarget.id ? { ...r, status: actionType } : r,
          ),
        );
      }
      closeAction();
  ...
  const toggleModalPaymentYear = (year: string) => {
    setModalOpenPaymentYears((prev) => {
      const next = new Set(prev);
      if (next.has(year)) next.delete(year);
      else next.add(year);
      return next;
      ...
      if (leaseModal === "terminate") {
        ...
        setActiveTenant((prev) =>
          prev
            ? {
                ...prev,
                leaseDurationMonths: data.data.request.leaseDurationMonths,
              }
            : prev,
        );
        setLeaseSuccess(
  ...
  const handleExtensionRespond = async (
    extensionId: number,
    ...
    try {
      const data = await ownerApi.respondToLeaseExtension(
        extensionId,
        decision,
      );
      if (!data.success) {
      ...
      setLeaseExtensions((prev) =>
        prev.map((e) =>
          e.id === extensionId ? { ...e, status: decision } : e,
        ),
      );
        ...
        setActiveTenant((prev) =>
          prev
            ? {
                ...prev,
                leaseDurationMonths:
                  prev.leaseDurationMonths + approved.requestedMonths,
              }
            : prev,
        );
      }
    ...
    try {
      ...
      if (newImageFiles.length > 0) {
        ...
        if (!imgData.success) {
          setSubmitMsg({
            type: "warning",
            text: "Property updated! Some images failed to upload.",
          });
      ...
      setSubmitMsg({
        type: "success",
        text: "Property updated successfully! Redirecting…",
      });
  ...
  const modalPaymentYears = Object.keys(modalPaymentsByYear).sort(
    ...
    <div className={styles.page}>
      ...
      {lightboxSrc && (
        <div className={styles.lightboxOverlay} onClick={closeLightbox}>
          ...
          {lightboxList.length > 1 && (
            <div
              className={styles.lightboxStrip}
              onClick={(e) => e.stopPropagation()}
            >
      ...
      {actionTarget && actionType && (
        <div className={styles.modalOverlay} onClick={closeAction}>
          <div className={styles.modal} onClick={(e) => e.stopPropagation()}>
            <div
              className={`${styles.reqModalHeader} ${actionType === "APPROVED" ? styles.reqModalHeaderApprove : styles.reqModalHeaderReject}`}
            >
            ...
            <div className={styles.reqModalBody}>
              <p className={styles.reqModalDesc}>
                {actionType === "APPROVED" ? (
                  <>
                    <strong>{actionTarget.tenantName}</strong>'s request will be
                    approved. They'll be notified by email. Other pending
                    requests will automatically be rejected.
                  </>
                ) : (
                  <>
                    <strong>{actionTarget.tenantName}</strong>'s request will be
                    rejected. They'll be notified by email.
                  </>
                )}
              </p>
              <div className={styles.reqModalMeta}>
                ...
                <span>
                  🗓 {actionTarget.leaseDurationMonths} month
                  {actionTarget.leaseDurationMonths !== 1 ? "s" : ""}
                </span>
              </div>
              {actionError && (
                <p className={styles.reqModalError}>⚠ {actionError}</p>
              )}
            </div>
      ...
      {leaseModal && activeTenant && (
        <div className={styles.modalOverlay} onClick={closeLeaseModal}>
          <div className={styles.modal} onClick={(e) => e.stopPropagation()}>
            <div
              className={`${styles.reqModalHeader} ${leaseModal === "terminate" ? styles.reqModalHeaderReject : styles.reqModalHeaderApprove}`}
            >
              <span>
                {leaseModal === "extend"
                  ? "➕"
              ...
              {leaseModal !== "terminate" ? (
                <>
                  <p className={styles.reqModalDesc}>
                    ...
                    for <strong>{activeTenant.tenantName}</strong>.{" "}
                    {leaseModal === "extend"
                  ...
                  >
                    ...
                    <span style={{ fontSize: 13, color: "#6e7071" }}>
                      month(s)
                    </span>
                  </div>
                  <div className={styles.reqModalMeta}>
                ...
                <>
                  <p className={styles.reqModalDesc}>
                    This will <strong>immediately terminate</strong> the lease
                    for <strong>{activeTenant.tenantName}</strong> and mark this
                    property as <strong>Available</strong>. The tenant will be
                    notified by email. This action{" "}
        ...
        <main className={styles.main}>
            ...
            {activeTenant ? (
              <div className={styles.visibilityLocked}>
                ...
                <div>
                  ...
                  <div className={styles.visibilityLockedSub}>
                    Cannot change visibility while a tenant is active.
                  </div>
                </div>
              </div>
            ) : canToggleStatus ? (
                  ...
                  {status === "UNAVAILABLE" && (
                    <div
                      style={{
                        ...
                        borderRadius: "10px",
                        fontSize: "13px",
                        background: isAdminDisabled
                          ? "rgba(192,57,43,0.08)"
                          : "#f9f9f9",
                        borderLeft: isAdminDisabled
                          ? "4px solid #c0392b"
                          : "4px solid #6e7071",
                      ...
                      {isAdminDisabled ? (
                        <div>
                          ...
                          <p style={{ margin: "4px 0 0", lineHeight: "1.5" }}>
                            ...
                            {adminNote ||
                              "Administrative restriction applied by platform management."}
                          </p>
                          <p
                            style={{
                              margin: "8px 0 0",
                        ...
                        <div>
                          <strong>OFFLINE:</strong> Manual deactivation by Owner.
                        </div>
                      )}
                    </div>
                  )}
                  <div
                    className={styles.visibilitySub}
                    style={{
                      marginTop: status === "UNAVAILABLE" ? "10px" : "4px",
                    }}
                  >
                    {status === "AVAILABLE"
              ...
              <div className={styles.visibilityLocked}>
                ...
                <div>
                  ...
                  <div className={styles.visibilityLockedSub}>
                    Visibility can only be toggled once approved.
                  </div>
                </div>
              </div>
            )}
              ...
              <div className={styles.activeTenantWrap}>
                <div className={styles.activeTenantRow}>
                  ...
                  <div className={styles.activeTenantInfo}>
                    <div className={styles.activeTenantName}>
                      {activeTenant.tenantName}
                    </div>
                    <div className={styles.activeTenantEmail}>
                      ✉️ {activeTenant.tenantEmail}
                    </div>
                    <SocialLinks
                      facebook={activeTenant.tenantFacebookUrl}
                      instagram={activeTenant.tenantInstagramUrl}
                      twitter={activeTenant.tenantTwitterUrl}
                    />
                  </div>
                ...
                <div className={styles.activeTenantStats}>
                  ...
                  <div className={styles.activeTenantStat}>
                    ...
                    <span className={styles.activeTenantStatValue}>
                      {activeTenant.leaseDurationMonths} month
                      {activeTenant.leaseDurationMonths !== 1 ? "s" : ""}
                    </span>
                  </div>
                  <div className={styles.activeTenantStat}>
                    <span className={styles.activeTenantStatIcon}>🏁</span>
                    ...
                    <span className={styles.activeTenantStatValue}>
                      {calcMoveOut(
                        activeTenant.startDate,
                        activeTenant.leaseDurationMonths,
                      )}
                    </span>
                  </div>
                </div>
                <div className={styles.activeTenantActions}>
                  <button
                    type="button"
                    className={styles.leaseExtendBtn}
                  ...
                  <button
                    type="button"
                    className={styles.leaseReduceBtn}
                  ...
                  <button
                    type="button"
                    className={styles.leaseTerminateBtn}
          ...
          <div className={styles.card}>
            <div className={styles.cardTitle}>
              Payment History{" "}
              {overdueCount > 0 && (
                <span className={styles.overduesBadge}>
            ...
            {!paymentsLoading && !paymentsError && payments.length > 0 && (
              <>
                <div className={styles.paymentSummaryStrip}>
                  ...
                  <div className={styles.paymentSummaryItem}>
                    <span
                      className={styles.paymentSummaryValue}
                      style={{ color: "#1a7a4a" }}
                    >
                      {paidCount}
                    </span>
                ...
                <div className={styles.yearAccordionList}>
                  {paymentYears.map((year) => {
                    ...
                    const yearPaid = yearPayments.filter(
                      (p) => p.status === "PAID",
                    ).length;
                    const yearOverdue = yearPayments.filter(
                      (p) => p.status === "OVERDUE",
                    ).length;
                    ...
                    return (
                      <div key={year} className={styles.yearAccordion}>
                        ...
                        >
                          <span className={styles.yearAccordionChevron}>
                            {isOpen ? "▾" : "▸"}
                          </span>
                          <span className={styles.yearAccordionLabel}>
                            {year}
                          </span>
                          <span className={styles.yearAccordionMeta}>
                            {yearPayments.length} payment
                            ...
                            · ₱
                            {yearTotal.toLocaleString("en-PH", {
                              minimumFractionDigits: 0,
                            })}
                          </span>
                        </button>
          ...
          <div className={styles.card}>
            ...
            {leaseExtLoading && (
              <div className={styles.requestsLoading}>
              ...
              >
                {leaseExtensions.map((ext) => {
                  ...
                  const bg = isApproved
                    ...
                    : ext.status === "REJECTED"
                      ? "rgba(192,57,43,0.06)"
                      : "rgba(183,142,66,0.06)";
                  const border = isApproved
                    ? "rgba(26,122,74,0.18)"
                    : ext.status === "REJECTED"
                      ? "rgba(192,57,43,0.18)"
                      : "rgba(183,142,66,0.18)";
                  return (
                    <div
                      key={ext.id}
                      style={{
                      ...
                      >
                        <div style={{ flex: 1 }}>
                          ...
                          >
                            ...
                            >
                              +{ext.requestedMonths} month
                              {ext.requestedMonths !== 1 ? "s" : ""} requested
                            </span>
                            <span
                              style={{
                                fontSize: "11px",
                        ...
                        {isPending && (
                          ...
                          >
                            <button
                              ...
                              disabled={isLoading}
                              onClick={() =>
                                handleExtensionRespond(ext.id, "REJECTED")
                              }
                              style={{
                                padding: "7px 14px",
                                borderRadius: "9px",
                            ...
                            <button
                              ...
                              disabled={isLoading}
                              onClick={() =>
                                handleExtensionRespond(ext.id, "APPROVED")
                              }
                              style={{
                                padding: "7px 14px",
                                borderRadius: "9px",
                    ...
                    {[5, 4, 3, 2, 1].map((star) => {
                      ...
                      return (
                        <div key={star} className={styles.reviewRatingBarRow}>
                          <span className={styles.reviewRatingBarLabel}>
                            {star}★
                          </span>
                          <div className={styles.reviewRatingBarTrack}>
                            <div
                              className={styles.reviewRatingBarFill}
                              style={{ width: `${pct}%` }}
                            />
                          </div>
                          <span className={styles.reviewRatingBarCount}>
                            {count}
                          </span>
                        </div>
                      );
          ...
          <div className={styles.card}>
            ...
            <div className={styles.thumbnailNote}>
              🖼 The <strong>first photo uploaded</strong> will be used as the
              listing thumbnail.
            </div>
            {visibleExisting.length > 0 && (
              <div className={styles.existingImagesWrap}>
                <p className={styles.existingImagesLabel}>
                  Current photos — click to preview
                </p>
            ...
            {totalPhotos < 10 && (
              ...
              >
                ...
                <div className={styles.imageUploadSub}>
                  ...
                  {10 - visibleExisting.length} more
                </div>
              </div>
            )}
              ...
              displayedRequests.length > 0 && (
                <div className={styles.yearAccordionList}>
                  {requestYears.map((year) => {
                    ...
                    const yearPending = yearReqs.filter(
                      (r) => r.status === "PENDING",
                    ).length;
                    return (
                      <div key={year} className={styles.yearAccordion}>
                        ...
                        >
                          ...
                          <span className={styles.yearAccordionMeta}>
                            {yearReqs.length} request
                            {yearReqs.length !== 1 ? "s" : ""}
                            {yearPending > 0 && (
                              <span style={{ color: "#b78e42" }}>
                                {" "}
                                · {yearPending} pending
                              </span>
                            )}
                                  ...
                                  >
                                    ...
                                    <div className={styles.requestInfo}>
                                      ...
                                      <div className={styles.requestMeta}>
                                        ...
                                        <span>
                                          🗓 {req.leaseDurationMonths} month
                                          {req.leaseDurationMonths !== 1
                                            ? "s"
                                            : ""}
                                        </span>
                                      ...
                                      <SocialLinks
                                        facebook={req.tenantFacebookUrl}
                                        instagram={req.tenantInstagramUrl}
                                        twitter={req.tenantTwitterUrl}
                                        size="sm"
                                      />
                                    </div>
                                    <div className={styles.requestRight}>
                                      <span
                                        ...
                                        style={{
                                          ...
                                          borderColor: statusColor(req.status),
                                        }}
                                      >
                                        {req.status}
                                      </span>
          ...
          {/* ── Past Request Modal (with Payment History) ── */}
          {pastRequestModal && (
            <div
              className={styles.modalOverlay}
              onClick={() => setPastRequestModal(null)}
            >
              <div
                className={styles.modal}
                onClick={(e) => e.stopPropagation()}
              ...
              >
                <div
                  className={`${styles.reqModalHeader}`}
                  style={{
                ...
                <div className={styles.reqModalBody} style={{ overflowY: "auto" }}>
                  <div
                    className={styles.reqModalMeta}
                    style={{ flexDirection: "column", gap: "10px" }}
                  >
                    <span>👤 {pastRequestModal.tenantName}</span>
                    <span>✉️ {pastRequestModal.tenantEmail}</span>
                    <span>📅 Move-in: {pastRequestModal.startDate}</span>
                    <span>
                      🗓 Lease: {pastRequestModal.leaseDurationMonths} month
                      {pastRequestModal.leaseDurationMonths !== 1 ? "s" : ""}
                    </span>
                    <span>
                      🕐 Submitted: {formatDate(pastRequestModal.createdAt)}
                    </span>
                    <span
                      style={{
                        color: statusColor(pastRequestModal.status),
                        fontWeight: 700,
                      }}
                    >
                      Status: {pastRequestModal.status}
                    </span>
                  </div>
                  
                  {(pastRequestModal.tenantFacebookUrl ||
                    pastRequestModal.tenantInstagramUrl ||
                    pastRequestModal.tenantTwitterUrl) && (
                    <div style={{ marginTop: "14px" }}>
                      ...
                      <SocialLinks
                        facebook={pastRequestModal.tenantFacebookUrl}
                        instagram={pastRequestModal.tenantInstagramUrl}
                        twitter={pastRequestModal.tenantTwitterUrl}
                        size="md"
                      />
                    </div>
                  )}
                  ...
                  <div style={{ marginTop: "24px" }}>
                    ...
                    {modalPaymentsLoading ? (
                      <div className={styles.requestsLoading} style={{ padding: "12px 0" }}>
                        Loading payment history…
                      </div>
                    ) : modalPayments.length === 0 ? (
                      <div className={styles.requestsEmpty} style={{ padding: "16px 0", background: "transparent", border: "none" }}>
                        <p>No payments recorded for this request.</p>
                      </div>
                    ) : (
                      <div className={styles.yearAccordionList}>
                          ...
                          return (
                            <div key={year} className={styles.yearAccordion}>
                              <button
                                type="button"
                                className={styles.yearAccordionHeader}
                                onClick={() => toggleModalPaymentYear(year)}
                              >
                                <span className={styles.yearAccordionChevron}>
                                  {isOpen ? "▾" : "▸"}
                                </span>
