...
const EditProperty: React.FC = () => {
  ...
  const handleMapClick = async (lat: number, lon: number) => {
    if (lat < CEBU_CITY_BOUNDS.minLat || lat > CEBU_CITY_BOUNDS.maxLat || lon < CEBU_CITY_BOUNDS.minLon || lon > CEBU_CITY_BOUNDS.maxLon) { setMapError("Please select a location within Cebu City."); return; }
    setMapSearching(true); setMapError(null);
    try {
    ...
    <div className={styles.page}>
      ...
      {lightboxSrc && (
        <div className={styles.lightboxOverlay} onClick={closeLightbox}>
          ...
          {lightboxList.length > 1 && <button className={`${styles.lightboxNav} ${styles.lightboxNavPrev}`} onClick={(e) => { e.stopPropagation(); goPrev(); }} type="button">‹</button>}
          <img src={lightboxSrc} alt="Full preview" className={styles.lightboxImg} onClick={(e) => e.stopPropagation()} />
          {lightboxList.length > 1 && <button className={`${styles.lightboxNav} ${styles.lightboxNavNext}`} onClick={(e) => { e.stopPropagation(); goNext(); }} type="button">›</button>}
            ...
            <div className={styles.reqModalBody}>
              {leaseModal !== "terminate" ? (
                ...
                <>
                  ...
                  <div className={styles.reqModalMeta}><span>👤 {activeTenant.tenantName}</span><span>✉️ {activeTenant.tenantEmail}</span><span>📅 {activeTenant.startDate}</span><span>🗓 {activeTenant.leaseDurationMonths} month(s)</span></div>
                </>
              )}
          ...
          <div className={styles.modal} onClick={(e) => e.stopPropagation()} style={{ maxHeight: "85vh", display: "flex", flexDirection: "column", maxWidth: "500px" }}>
            ...
            <div className={styles.reqModalBody} style={{ overflowY: "auto", padding: "20px 24px" }}>
              <div className={styles.reqModalMeta} style={{ flexDirection: "column", gap: "10px" }}>
                ...
                <span>🗓 Lease: {pastRequestModal.leaseDurationMonths} month{pastRequestModal.leaseDurationMonths !== 1 ? "s" : ""}</span>
                <span>🕐 Submitted: {formatDate(pastRequestModal.createdAt)}</span>
          ...
          {activeTab === "property" && (
            <>
              ...
              <div className={styles.card}>
                ...
                {activeTenant ? (
                  <div className={styles.visibilityLocked}><span className={styles.visibilityLockedIcon}>🔒</span><div><div className={styles.visibilityLockedLabel}>Property is <strong>Occupied</strong></div><div className={styles.visibilityLockedSub}>Cannot change visibility while a tenant is active.</div></div></div>
                ) : canToggleStatus ? (
                  <div className={styles.visibilityWrap}>
                    <div className={styles.visibilityInfo}>
                      <div className={styles.visibilityLabel}>{status === "AVAILABLE" ? "🟢 Visible on listings" : "🔴 Hidden from listings"}</div>
                      {status === "UNAVAILABLE" && (
            ...
            <>
              ...
              <div className={styles.card}>
                ...
                {activeTenantLoading ? (
                  ...
                  <div className={styles.requestsEmpty}><span className={styles.requestsEmptyIcon}>🏠</span><p>No active tenant. Property is currently vacant.</p></div>
                ) : (
                                ...
                                return (
                                  <div key={req.id} className={styles.requestRow} onClick={!isPending ? () => setPastRequestModal(req) : undefined} style={{ cursor: !isPending ? "pointer" : undefined }}>
                                    ...
                                    <div className={styles.requestInfo}>
                                      ...
                                      <div className={styles.requestMeta}><span>✉️ {req.tenantEmail}</span><span>📅 Move in: {req.startDate}</span><span>🗓 {req.leaseDurationMonths} month{req.leaseDurationMonths !== 1 ? "s" : ""}</span><span>🕐 {timeAgo(req.createdAt)}</span></div>
          ...
          {activeTab === "reviews" && (
            <div className={styles.card}>
              <div className={styles.cardTitle} style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                <div>
                  Tenant Reviews{" "}
                  {avgRating && <span className={styles.reviewsAvgBadge}>★ {avgRating} · {reviews.length} review{reviews.length !== 1 ? "s" : ""}</span>}
                </div>
              </div>
